...
void OLED_Init(void)
{
	...
	OLED_Clear();	}			
void OLED_ShowFloatNum(uint8_t Line, uint8_t Column, double Number, uint8_t DecimalDigits) {
    if (Number < 0) {
        OLED_ShowChar(Line, Column++, '-');
        Number = -Number;
    }
    ...
    for (int i = IntegerDigits - 1; i >= 0; i--) {
        long divisor = OLED_Pow(10, i);
        uint8_t digit = (integer_part / divisor) % 10;
    ...
    for (int i = 0; i < DecimalDigits; i++) {
        fraction_part *= 10;
        uint8_t digit = (uint8_t)(fraction_part + 0.5);
